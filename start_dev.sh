...
BASE_PATH=$(pwd)
FRONTEND_PATH="$BASE_PATH/app/frontend"
DATABASE_APP="$BASE_PATH/database_project_folder"
CONTRACT_TESTS="$BASE_PATH/app"
COMMAND_LEFT_TOP=""
COMMAND_RIGHT_TOP=""
...
tmux send-keys -t $SESSION_NAME:0.1 "cd $FRONTEND_PATH" C-m
tmux send-keys -t $SESSION_NAME:0.1 "$COMMAND_RIGHT_TOP" C-m

# 0.2 Right Bottom
tmux split-window -v -t $SESSION_NAME:0.1
tmux send-keys -t $SESSION_NAME:0.2 "cd $CONTRACT_TESTS" C-m
